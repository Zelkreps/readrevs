    ...
    private var header: some View {
        HStack(spacing: 14) {
            ...
            VStack(alignment: .leading, spacing: 3) {
                ...
                Text(
                    "\(model.appName) | \(model.reviewCount.formatted()) reviews | "
                        + "\(model.storefrontCount) storefronts"
                )
                .font(.caption)
            ...
            .help("Open previous ReadRevs analyses")
            ...
            AppCloseButton(
                ...
                help: model.isRunning
                    ? "Close; analysis continues in the background"
                    : "Close research",
                action: sessionManager.dismissPresentation
            )
        }
        .padding(20)
    }
    ...
    private var researchContext: some View {
        HStack(alignment: .top, spacing: 10) {
            Image(systemName: "lock.shield")
                .foregroundStyle(.secondary)
            Text(
                "Codex reads an isolated, read-only export. Review text is untrusted data "
        ...
        VStack(alignment: .leading, spacing: 10) {
            ...
            Text(
                self.model.messages.isEmpty
                    ...
                    : "Uses the Codex CLI account signed in on this Mac. Press Command-Return to send."
            )
            .font(.caption2)
...
struct CodexResearchCenterPopover: View {
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 0) {
            HStack {
                Label("ReadRevs", systemImage: "clock.arrow.circlepath")
                    .font(.headline)
                Spacer()
                if totalCount > 0 {
            ...
            if sessions.isEmpty, savedEntries.isEmpty, errorMessage == nil {
                ...
                ScrollView {
                    LazyVStack(alignment: .leading, spacing: 0) {
                        ...
                        if let errorMessage {
                            if !sessions.isEmpty || !savedEntries.isEmpty { Divider() }
    ...
    var body: some View {
        HStack(alignment: .top, spacing: 12) {
            ...
            VStack(alignment: .leading, spacing: 5) {
                ...
                Text(
                    "\(session.reviewCount.formatted()) reviews | "
                        + "\(session.codexModel.displayName) | \(session.reasoningEffort.title)"
                )
                .font(.caption)
                .foregroundStyle(.secondary)
                .lineLimit(1)
    ...
    var body: some View {
        HStack(alignment: .top, spacing: 12) {
            ...
            VStack(alignment: .leading, spacing: 5) {
                ...
                Text(
                    "\(entry.reviewCount.formatted()) reviews | "
                        + "\(entry.codexModel.displayName) | \(entry.reasoningEffort.title)"
                )
                .font(.caption)
                .foregroundStyle(.secondary)
                .lineLimit(1)
    ...
    var body: some View {
        VStack(spacing: 0) {
            HStack(spacing: 14) {
                ...
                VStack(alignment: .leading, spacing: 3) {
                    Text("ReadRevs: \(entry.appName)")
                        .font(.title3.weight(.semibold))
                    Text(
                        "Saved \(entry.updatedAt.formatted(date: .abbreviated, time: .shortened)) | "
                            + "\(entry.reviewCount.formatted()) reviews"
                    )
                    .font(.caption)
                    .foregroundStyle(.secondary)
