...
struct CodexResearchBundleService {
    ...
    func prepare(
        ...
        let directory = rootDirectory.appending(
            path: "\(app.appID)-\(safePathComponent(identifier))",
    ...
    private func researchBrief(
        ...
        let reviewLabel = reviews.count == 1 ? "1 synced written review" : "\(reviews.count) synced written reviews"
        return """
        # ReadRevs research bundle

        - App: \(app.name)
        - App Store ID: \(app.appID)
        - Version at export: \(app.version)
        - Dataset: \(reviewLabel)
